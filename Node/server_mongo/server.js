...
var student = mongoose.Schema({ // student라는 테이블 생성
    name: 'string',
    address: 'string',
    age: 'number'
});
...
// 12. 특정아이디 수정하기
Student.findById({ _id: '5f9f59cd484f2a18b4795447' }, function (error, student) {
...
// 13. 삭제
Student.remove({ _id: '5f9f59cd484f2a18b4795447' }, function (error, output) {
    console.log('--- Delete ---');
    if (error) {
        console.log(error);
    }
    /*
     ( SINCE DELETE OPERATION IS IDEMPOTENT, NO NEED TO SPECIFY )
        어떤 과정을 반복적으로 수행 하여도 결과가 동일하다. 
        삭제한 데이터를 다시 삭제하더라도, 존재하지 않는 데이터를 제거요청 하더라도 오류가 아니기 때문에
